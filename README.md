...
Second, `Module`s should not use static references to `Scope`s, but instead accept `Scope`s as their constructor params. In case of most technologies, usually 2 types of `Scope`s make sense: a short-term one for storing stuff like enclosing transactions (DB / Entity / distributed) and a long-term one for storing stuff like auth-tokens, client conversation state (for example a shopping cart) etc.
